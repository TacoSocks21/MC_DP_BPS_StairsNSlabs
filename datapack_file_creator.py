...
backport_mod_minecraft_version = "1.21.5"
...
# Loop through each item namespace
for namespace, namespace_item_types in custom_recipe_items.items():
            ...
            for recipe_type, recipe in recipes.items():
                if item_type == "wooden":
                    ...
                    if recipe_type[-5:] == "block":
                        ...
                        recipe_file_name = item + '_' + \
                            recipe_type[:-5] + "planks"
                    elif recipe_type[:5] == "block":
                        # Change wording from "block" to "planks" for wooden items
                        recipe_file_name = item + '_' + \
                            "planks" + recipe_type[5:]
                    else:
                ...
                # **************************************************
                # Change vanilla/mod recipe for stairs
                if recipe_type == "stairs_from_block":
                    # Replace vanilla/mod result from 4 to 8 stairs to match datapack
                    sorted_recipe["pattern"] = ["#  ", "## ", "###"]
                    sorted_recipe["result"]["count"] = 8
                    with open(f"./minecraft_versions/{minecraft_version}/data/{namespace}/recipe/{item}_stairs.json", 'w') as json_file:
                        json.dump(sorted_recipe, json_file, indent=4)

            # **************************************************
            # Other vanilla recipe overrides
            if item in minecraft_recipe_overrides[namespace]["pressure_plate"][item_type]:
                ...
                if pack_format_values[minecraft_version] == 48:
                    ...
                    recipes["pressure_plate"] = {
                        ...
                        "ingredient": {
                            "item": f"{namespace}:{item}_planks" if item in minecraft_recipe_overrides[namespace]["pressure_plate"]["wooden"] else f"{namespace}:{item}"
                        },
                        "result": {
                            "id": f"{namespace}:{item}_pressure_plate",
                            "count": 9
                        }
                    }
                    ...
                    recipes["pressure_plate"] = {
                        "type": "minecraft:stonecutting",
                        "ingredient": [
                            f"{namespace}:{item}_planks" if item in minecraft_recipe_overrides[namespace]["pressure_plate"]["wooden"] else f"{namespace}:{item}"
                        ],
                        "result": {
                            "id": f"{namespace}:{item}_pressure_plate",
                            "count": 9
